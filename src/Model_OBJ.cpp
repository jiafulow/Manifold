...
void Model_OBJ::Calc_Bounding_Box(glm::dvec3& min_corner, glm::dvec3& max_corner)
{
...
void Model_OBJ::Build_Tree(int resolution)
{
    ...
    Calc_Bounding_Box(min_corner, max_corner);
...
{
    glm::dvec3 cpoint = glm::dvec3(1e30,1e30,1e30);
    glm::dvec3 tris[3];
    glm::dvec3 normal;
    for (set<int>::const_iterator it = v_faces[i].begin();
        it != v_faces[i].end(); ++it)
    {
        int face_ind = *it;
        for (int j = 0; j < 3; ++j)
...
//  Build_BVH();
    ...
    if (flag != 0)
    {
        ...
        //os << fn << "\n";
        //os.close();
        cout << "Error: Not a Manifold! " << flag << "\n";
        exit(0);
    }
}

bool Model_OBJ::Split_Grid(map<Grid_Index,int>& vcolor, vector<glm::dvec3>& nvertices, const vector<glm::ivec4>& nface_indices, vector<set<int> >& v_faces, vector<glm::ivec3>& triangles)
{
    double unit_len = 0;
